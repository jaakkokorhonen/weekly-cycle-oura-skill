...
Tämän tarkoitus on erottaa yksi pitkä työjakso useasta fragmentoituneesta jaksosta, koska juuri tätä hypoteesia skill testaa.

#### `detect_main_meal(day_events)`
...
#### `compute_caffeine_window(day_events, sleep_sessions)`
...
#### `compute_alcohol_window(day_events, sleep_sessions)`
...
#### `segment_sleep_night(sleep_session)`
...
Tämä on keskeinen lisälogiikka, koska Oura itsessään ei tulkitse historiallisesti tai kokeellisesti kahta unta vaihtoehtoisena rakenteena, vaan arvioi pääasiassa yhden yön tehokkuutta.

#### `compute_recovery_cost(day_data)`
...
#### `compute_capacity_signal(week_data)`
...
### 2. Alkoholi ensimmäisen unijakson käynnistäjänä
...
### 3. Päiväuni ehdollisena korjausliikkeenä
...
### 6. Lämmin ateria rytmin ankkurina
